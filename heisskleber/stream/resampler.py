...
def round_dt(dt, delta):
    # Could use math.ceil to always return the next higher timestamp
    # Or round to return the nearest timestamp
...
def timestamp_generator(start_epoch, timedelta_in_ms):
    ...
    # Start with the next full delta, add delta, because round_dt uses floor
    next = round_dt(timestamp_start, delta) + delta_half
    yield_next = datetime.timestamp(next)
    yield yield_next
...
class Resampler:
    ...
    async def resample(self) -> dict[str, Serializable]:
        aggregated_data = []
...
async def main():
    ...
    asyncio.create_task(sub1.start_loop())
    asyncio.create_task(sub2.start_loop())
    asyncio.create_task(resampler1.run())
    asyncio.create_task(resampler2.run())
    ...
    while True:
        m1 = await anext(gen1)  # noqa
        m2 = await anext(gen2)  # noqa

        print(f"epoch: {m1['epoch']}")
        # print(f"acc - roll: {m1['acc_x'] - m2['roll']}")
        # print(f"time diff: {time2 - time1}")
